...
public:
    ...
    void UpdateNSFromPose(const cv::Mat &Tco);
    
    const NavState& GetNavState(void) const;
    void SetNavState(const NavState& ns);
    void UpdateNavState(ICPer &icper);
    
    // point clouds near to this frame
    ...
    void SetLidarPose(cv::Mat lidarpose);
    
    // Incremental Lidar pose.
    cv::Mat mTlw;
