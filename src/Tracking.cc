...
Tracking::Tracking(System* pSys, ORBVocabulary* pVoc, FrameDrawer* pFrameDrawer, MapDrawer* pMapDrawer, Map* pMap, KeyFrameDatabase* pKFDB, const string& strSettingPath, const int sensor, ConfigParam* pParams, LidarMap* pLidarMap):
...
Tracking::Tracking(System* pSys, ORBVocabulary* pVoc, FrameDrawer* pFrameDrawer, MapDrawer* pMapDrawer, Map* pMap, KeyFrameDatabase* pKFDB, const string& strSettingPath, const int sensor, ConfigParam* pParams, LidarMap* pLidarMap, KeyFrameDatabase* pMapDB):
mState(NO_IMAGES_YET), mSensor(sensor), mbOnlyTracking(false), mbVO(false), mpORBVocabulary(pVoc),
mpKeyFrameDB(pKFDB), mpInitializer(static_cast<Initializer*>(NULL)), mpSystem(pSys), mpViewer(NULL),
mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0), mpLidarMap(pLidarMap), globalOptimized(false),
...
bool Tracking::ModelBasedRelocalization()
{
...
// 	mCurrentFrame.SetPose(kfpair.Twc);
	...
	double ratio = static_cast<double>(TotalNum)/static_cast<double>(mCurrentFrame.N);
	cout << "Total Model Points " << TotalNum << ", with ratio " << ratio << endl;

	if(ratio > 0.1)
	{
	    cout << "Ready for tracking with model" << endl;
	    mCurrentFrame.bReadyForModel = true;
...
void Tracking::PredictNavStateByLidar(bool bMapUpdated)
{
	...
	icper.reset();
	icper.process(mpLastKeyFrame->pCloud, mCurrentFrame.pCloud);
	
        // Get initial NavState&pose from Last KeyFrame
	mCurrentFrame.SetNavState(mpLastKeyFrame->GetNavState());
	mCurrentFrame.UpdateNavState(icper);
        mCurrentFrame.UpdatePoseFromNS(ConfigParam::GetMatToc());
    ...
    {
        icper.reset();
	icper.process(mLastFrame.pCloud, mCurrentFrame.pCloud);
	
        // Get initial pose from Last Frame
	mCurrentFrame.SetNavState(mLastFrame.GetNavState());
        mCurrentFrame.UpdateNavState(icper);
        mCurrentFrame.UpdatePoseFromNS(ConfigParam::GetMatToc());
    }
}

bool Tracking::TrackWithLidar(bool bMapUpdated)
{
    ORBmatcher matcher(0.9,true);
    
    Frame mCurrentFrameCopy = mCurrentFrame;
    
    UpdateLastFrame();

    mCurrentFrame.SetPose(mVelocity*mLastFrame.mTcw);
    mCurrentFrame.UpdateNSFromPose(ConfigParam::GetMatT_co());
    ...
    if(nmatchesMap < 100)
    {
	cout << "Weak tracking using vision, try Lidar instead" << endl;
    }
...
cv::Mat Tracking::GrabImageStereoLidar(const cv::Mat& imLeft, 
				       const cv::Mat& imRight, 
				       shared_ptr< DP > pCloud, 
				       const double& timestamp)
{
	...
	mCurrentFrame = Frame(mImGray, imGrayRight, timestamp, mpORBextractorLeft,mpORBextractorRight, pCloud,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);
    else
	mCurrentFrame = Frame(mImGray, imGrayRight, timestamp, mpORBextractorLeft,mpORBextractorRight, pCloud,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpLastKeyFrame);
    
    Track();
    return mCurrentFrame.mTcw.clone();
}

void Tracking::DrawCurrentLidarPoints()
{
    mCurrentFrame.DrawLidarPoints();
}
...
void Tracking::Track()
{
    ...
    unique_lock<mutex> lock1(mpLidarMap->mMutexMapUpdate);
...
// 	    {
// 	      cout << "Create Map Database" << endl;
// 	      for(int i = 0;i < 10/*mpMap->mvKFItems.size()*/; i++)
// 	      {
// 		  KFPair tempKFp = mpMap->mvKFItems[i];
// 		  KFItem tempKFi;
// 		  tempKFi.score = 0;
// 		  tempKFi.Twc = tempKFp.Twc;
// 		  tempKFi.Twl = tempKFp.Twl;
// 		  tempKFi.vBow = tempKFp.vBow;
// 		  mpMapDB->add(tempKFi);
// 	      }
// 	      cout << "Finish" << endl;
// 	      isInitMapDB = true;
...
// 	    }
        ...
        {
// 	    cout << "Localization with map" << endl;
        }
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    KeyFrame* pKFcur = new KeyFrame(mCurrentFrame,mpMap,mpKeyFrameDB, NULL);
...
//     mCurrentFrame.SetNavState(Converter::toSE3NS(mLastFrame.mTcw.inv()*ConfigParam::GetMatT_co()));
    mCurrentFrame.UpdateNSFromPose(ConfigParam::GetMatT_co());
...
//     mLastFrame.SetNavState(Converter::toSE3NS(mLastFrame.mTcw.inv()*ConfigParam::GetMatT_co()));
    mLastFrame.UpdateNSFromPose(ConfigParam::GetMatT_co());
...
void Tracking::CreateNewKeyFrame()
{
    ...
    pKF->SetNavState(mCurrentFrame.GetNavState());
