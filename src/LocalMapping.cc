...
namespace ORB_SLAM2
{
LocalMapping::LocalMapping(Map* pMap, const float bMonocular, ConfigParam* pParams, LidarMap* pLidarMap):
    mbMonocular(bMonocular), mbResetRequested(false), mbFinishRequested(false), mbFinished(true), mpMap(pMap),
    mbAbortBA(false), mbStopped(false), mbStopRequested(false), mbNotStop(false), mbAcceptKeyFrames(true),
    mpLidarMap(pLidarMap)
{
    mpParams = pParams;
	    ...
	    if(!CheckNewKeyFrames() && !stopRequested())
            {
                ...
                {
		    Optimizer::LocalBundleAdjustmentNavState(mpCurrentKeyFrame,icper, mlLocalKeyFrames,&mbAbortBA, mpMap, this);
                }
...
void LocalMapping::ProcessNewKeyFrame()
{
    ...
    mpLidarMap->AddKeyFrame(mpCurrentKeyFrame);
}
