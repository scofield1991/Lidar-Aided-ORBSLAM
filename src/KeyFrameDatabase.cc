...
void KeyFrameDatabase::add(KFItem pKFI)
{
    unique_lock<mutex> lock(mMutex);

    for(DBoW2::BowVector::const_iterator vit= pKFI.vBow.begin(), vend=pKFI.vBow.end(); vit!=vend; vit++)
    {
        mvInvPoseFile[vit->first].push_back(pKFI);
    }
}
...
void KeyFrameDatabase::erase(KFItem pKFI)
{
    ...
    DBoW2::BowVector vBowV = pKFI.vBow;
...
vector< KFItem > KeyFrameDatabase::DetectInitLocalization(Frame* F)
{
    list<KFItem> lSPsSharingWords;
    ...
    // Search all score pairs that share a word with current frame
    {
        ...
        {
            list<KFItem> &lSPs =   mvInvPoseFile[vit->first];

            for(list<KFItem>::iterator lit=lSPs.begin(), lend= lSPs.end(); lit!=lend; lit++)
            {
                lit->score++;
    ...
    if(lSPsSharingWords.empty())
        return vector<KFItem>();

    // Only compare against those keyframes that share enough words
    int maxCommonWords=0;
    for(list<KFItem>::iterator lit=lSPsSharingWords.begin(), lend= lSPsSharingWords.end(); lit!=lend; lit++)
    {
    ...
    list<pair<float,KFItem> > lScoreAndMatch;
...
//     list<pair<float,KFItem> > lAccScoreAndMatch;
//     float bestAccScore = 0;
// 
//     // Lets now accumulate score by covisibility
//     for(list<pair<float,KFItem> >::iterator it=lScoreAndMatch.begin(), itend=lScoreAndMatch.end(); it!=itend; it++)
//     {
//         KFItem pKFi = it->second;
//         vector<KFItem> vpNeighs = pKFi->GetBestCovisibilityKeyFrames(10);
// 
...
//     }
    ...
    set<KFItem> spAlreadyAddedKF;
    vector<KFItem> vpRelocCandidates;
