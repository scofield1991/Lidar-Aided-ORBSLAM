...
// void Optimizer::PnPinitilization()
// {
...
//     pFrame->SetPose(pose);
//     
// //     pFrame->SetNavState(Converter::toSE3NS(pFrame->mTcw.inv()*ConfigParam::GetMatT_co()));
//     pFrame->UpdateNSFromPose(ConfigParam::GetMatT_co());
//    
//     return nInitialCorrespondences-nBad;
// }
  
void Optimizer::GlobalBundleAdjustmentNavState(Map* pMap, ICPer &icper, int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
	    ...
	    icper.reset();
	    icper.process(pKF0->pCloud, pKF1->pCloud);
	    
	    NavState meas;
	    meas.Set_Pos(icper.getDeltaP());
	    meas.Set_Rot(icper.getDeltaR());
	    epvr->setMeasurement(meas);
	    
            Matrix6d InvCovPVR = icper.getCovPPhi().inverse();
...
int Optimizer::PoseOptimization(Frame *pFrame, Frame* pLastFrame, ICPer& icper, const bool& bComputeMarg)
{
    // Extrinsics
    Matrix4d Toc = ConfigParam::GetEigToc();
    Matrix3d Roc = Toc.topLeftCorner(3,3);
    ...
    g2o::EdgeNavStatePrior* eNSPrior = new g2o::EdgeNavStatePrior();
    {
        ...
        const float thHuberNavState = sqrt(16.812);
        g2o::RobustKernelHuber* rk = new g2o::RobustKernelHuber;
...
// 	icper.reset();
// 	icper.process(pLastFrame->pCloud, pFrame->pCloud);

	NavState meas;
	meas.Set_Pos(icper.getDeltaP());
	meas.Set_Rot(icper.getDeltaR());
	eNSPVR->setMeasurement(meas);
...
//                 cerr<<"stereo shouldn't in poseoptimization"<<endl;
		nInitialCorrespondences++;
                ...
                e->SetParams(pLastFrame->fx,pLastFrame->fy,pFrame->cx,pLastFrame->cy,pLastFrame->mbf,Roc,Poc,Converter::toVector3d(pMP->GetWorldPos()));
...
int Optimizer::PoseOptimization(Frame *pFrame, KeyFrame* pLastKF, ICPer& icper, const bool& bComputeMarg)
{
    // Extrinsics
    Matrix4d Toc = ConfigParam::GetEigToc();
    Matrix3d Roc = Toc.topLeftCorner(3,3);
...
// 	icper.reset();
// 	icper.process(pLastKF->pCloud, pFrame->pCloud);
...
void Optimizer::LocalBundleAdjustmentNavState(KeyFrame *pKF, ICPer &icper, 
					      ...
					      bool* pbStopFlag, Map* pMap, LocalMapping* pLM)
{
    ...
    //Model Points 
    list<MapPoint*> lModelPoints;
    for(list<KeyFrame*>::const_iterator lit=lLocalKeyFrames.begin() , lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
        vector<MapPoint*> vpMPs = (*lit)->mvpModelPoints;
        for(vector<MapPoint*>::iterator vit=vpMPs.begin(), vend=vpMPs.end(); vit!=vend; vit++)
        {
            MapPoint* pMP = *vit;
	    if(pMP)
    ...
    // Test log
    else if(lLocalKeyFrames.front()->mnId != 0){cerr<<"pKFPrevLocal is NULL?"<<endl;}
    // Covisible KeyFrames
    ...
    // Set MapPoint vertices
    const int nExpectedSize = (lLocalKeyFrames.size()+lFixedCameras.size())*(lLocalMapPoints.size()+lModelPoints.size());

    vector<g2o::EdgeNavStatePRStereoPointXYZ*> vpEdgesStereo;
    vpEdgesStereo.reserve(nExpectedSize);
    
    vector<g2o::EdgeNavStatePRStereoPointXYZ*> vpEdgesModelStereo;
    vpEdgesModelStereo.reserve(nExpectedSize);

    vector<KeyFrame*> vpEdgeKFStereo;
    vpEdgeKFStereo.reserve(nExpectedSize);
    
    vector<KeyFrame*> vpEdgeKFModelStereo;
    vpEdgeKFModelStereo.reserve(nExpectedSize);

    vector<MapPoint*> vpMapPointEdgeStereo;
    vpMapPointEdgeStereo.reserve(nExpectedSize);
    
    vector<MapPoint*> vpModelPointEdgeStereo;
    vpModelPointEdgeStereo.reserve(nExpectedSize);
...
//     const float thHuberMono = sqrt(5.991);
    ...
    //Model Based Method
    for(list<MapPoint*>::iterator lit=lModelPoints.begin(), lend=lModelPoints.end(); lit!=lend; lit++)
    {
        MapPoint* pMP = *lit;
        g2o::VertexSBAPointXYZ* vPoint = new g2o::VertexSBAPointXYZ();
        vPoint->setEstimate(Converter::toVector3d(pMP->GetWorldPos()));
        ...
        vPoint->setId(id);
        vPoint->setMarginalized(true);
	vPoint->setFixed(true);// Special for Model Points
        optimizer.addVertex(vPoint);

        const map<KeyFrame*,size_t> observations = pMP->GetObservations();

        // Set edges between KeyFrame and Model Points
        for(map<KeyFrame*,size_t>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            KeyFrame* pKFi = mit->first;
		...
		const float &kp_ur = pKFi->mvuRight[mit->second];
		    ...
		    vpEdgesModelStereo.push_back(e);
		    vpEdgeKFModelStereo.push_back(pKFi);
		    vpModelPointEdgeStereo.push_back(pMP);
                }
            }
        }
    }
    ...
    for(size_t i=0, iend=vpEdgesModelStereo.size(); i<iend;i++)
    {
        g2o::EdgeNavStatePRStereoPointXYZ* e = vpEdgesModelStereo[i];
        MapPoint* pMP = vpModelPointEdgeStereo[i];
				       ...
				       LoopClosing* pLC)
{
    ...
    cv::Mat Toc = ConfigParam::GetMatT_co();
